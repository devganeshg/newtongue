<p align="center">
  <img src="assets/logo.svg" width="96" alt="Newtongue logo">
</p>

<h1 align="center">Newtongue</h1>

<p align="center"><b>Dub any video into another language</b> — natural neural voices, no API keys.<br>
Windows · macOS · Linux</p>

<p align="center">
  <a href="https://github.com/devganeshg/newtongue/actions/workflows/ci.yml"><img src="https://github.com/devganeshg/newtongue/actions/workflows/ci.yml/badge.svg" alt="CI status"></a>
  <a href="LICENSE"><img src="https://img.shields.io/badge/license-MIT-blue.svg" alt="MIT license"></a>
  <a href="https://github.com/devganeshg/newtongue/releases"><img src="https://img.shields.io/github/v/release/devganeshg/newtongue?include_prereleases" alt="Latest release"></a>
...
- **🗣️ 15 languages, hundreds of voices** — English, Hindi, Spanish, French, German,
  ...
  edge-tts voice can be substituted (`newtongue --list-voices hi`).
...
- **✍️ Edit, then dub** — feed Newtongue your own (or hand-corrected) SRT/VTT with
...
- **🌍 Dub into multiple languages in one run** — pick several target languages at
  once; Newtongue transcribes the source audio only once and reuses it for every
...
- **🛡️ Voice fallback** — if the free TTS endpoint rejects a specific text+voice combo,
  Newtongue automatically retries with other voices of the same locale and tells you.
- **🔇 Lossless video** — the video stream is copied untouched (no re-encode, no quality
  loss); only the audio track is replaced.
- **🖥️ Web UI and CLI** — a friendly browser app for everyday use, a `newtongue` command
...
| OS | Do this |
|---|---|
| **Windows** | Double-click `Start Newtongue.bat` |
| **macOS** | Right-click `Start Newtongue.command` → **Open** (only needed the first time; after that, double-click) |
| **Linux** | `./start-newtongue.sh` |
...
- **`uv sync` fails partway through** — often a path-length problem: Windows limits paths to
  ...
  unzipped Newtongue folder somewhere short, e.g. `C:\Newtongue`, and re-run.
...
- **"cannot be opened because it is from an unidentified developer"** — right-click
  `Start Newtongue.command` → **Open** (only needed the first time).
...
- **"Permission denied" running the script** — `chmod +x start-newtongue.sh` first.
...
- **Dependency install or first dub fails with a network error** — Newtongue needs internet
...
- Still stuck? [Open an issue](https://github.com/devganeshg/newtongue/issues/new) with the exact
...
```bash
uv run newtongue input.mp4 --to hi --srt                    # dub to Hindi + subtitles
uv run newtongue input.mp4 --to es --keep-bg                # keep music/ambience underneath
uv run newtongue input.mp4 --to ja --model medium            # better transcription, slower
uv run newtongue --list-voices th                            # see voices for a language
uv run newtongue input.mp4 --to zh-TW --voice zh-TW-YunJheNeural
uv run newtongue input.mp4 --to hi,es,ja --subtitles srt,vtt # dub into 3 languages in one pass
uv run newtongue input.mp4 --to hi --burn-subtitles          # hardcode subtitles into the video
uv run newtongue input.mp4 --to hi --embed-subtitles         # soft subtitle track (toggleable, lossless)
uv run newtongue input.mp4 --to hi --srt --subtitle-content both  # bilingual subtitles
uv run newtongue input.mp4 --to hi --no-dub --subtitles srt  # translated subtitles only, no dubbing
uv run newtongue input.mp4 --to hi --from-subs fixed.srt     # dub from your hand-corrected transcript
uv run newtongue --no-dub --from-subs movie.srt --to es      # translate a subtitle file (no video)
uv run newtongue input.mp4 --to hi --rate +15% --pitch -10Hz # faster, deeper voice
```
...
(`nt` is a shorter alias for the same command.)
...
see `newtongue/voices.py` to add more (any edge-tts locale works; add a line with
...
- `--burn-subtitles` re-encodes the video (subtitle burn-in can't be a lossless copy) and
  ...
  it, Newtongue automatically falls back to the auto-downloaded static build, which has it.
- **Burned subtitles showing □□□ boxes instead of text?** That means no font with the
  target script's glyphs was found. Newtongue automatically picks an installed font that
...
- **Embedded subtitles unreadable in your player?** Some players (notably QuickTime)
  ...
  `--burn-subtitles` instead (drawn by Newtongue itself, works everywhere), or play the
  video+SRT pair in VLC/mpv.
- `--rate`/`--pitch`/`--volume` adjust the synthesized voice itself; they're independent of
  the automatic per-segment speed-up Newtongue applies to keep segments from overlapping.
...
- 🐛 Found a bug? [Open an issue](https://github.com/devganeshg/newtongue/issues/new/choose).
- 💡 Have an idea? [Start a discussion](https://github.com/devganeshg/newtongue/discussions) or
  open a feature-request issue.
- 🔀 Ready to code? Fork the repo, make your change, and open a pull request.

## Third-party services and your responsibilities

Newtongue is a client. It does not host translation or speech synthesis — it talks to services
...
stop working at any time without notice — that is the single most likely reason Newtongue will
...
personality/voice rights depending on your jurisdiction. Newtongue's authors are not
responsible for how you use it.

**Machine translation is not accurate translation.** Do not rely on Newtongue's output for
legal, medical, safety, or other consequential material without human review.

## License

Newtongue itself is [MIT licensed](LICENSE) — use it, modify it, and ship it however you like.

It depends on third-party projects under their own licenses, and two are worth calling out
because they are **not** permissive:

- **[edge-tts](https://github.com/rany2/edge-tts) is LGPL-3.0.** Newtongue imports it as an
  unmodified, separately-installed library, so Newtongue stays MIT. If you redistribute Newtongue
...
- **ffmpeg builds are typically GPL-licensed.** Newtongue does not ship ffmpeg; it uses your
  system ffmpeg, or `static-ffmpeg` downloads a build onto *your* machine at runtime, so
  plain use carries no distribution obligation. If you package Newtongue into an installer that
